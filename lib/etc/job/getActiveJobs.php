<?php
    ...
    // Get a full list of jobs within a date span (start and end dates are included) (overdue jobs do not have to be within the date span!)
		...
		function getActiveJobs($startDateTime, $endDateTime, array $exclude = array()) { // "singular", "recurring", "completed", "cancelled", "overdue"
            ...
            $fetch = $db->select('job', '*', "WHERE businessId = '".$_SESSION['ultiscape_businessId']."' AND ((startDateTime >= '".$startDateTimeSan."' AND endDateTime <= '".$startDateTimeSan."') OR (startDateTime >= '".$startDateTimeSan."' AND endDateTime = NULL) OR (endDateTime >= '".$startDateTimeSan."' AND endDateTime <= '".$endDateTimeSan."') OR (endDateTime >= '".$startDateTimeSan."' AND endDateTime = NULL) OR (frequencyInterval != 'none' AND startDateTime <= '".$startDateTimeSan."' AND endDateTime <= '".$endDateTimeSan."' ))");
            if ($fetch) {
                $jobs = array_merge($jobs, $fetch);
            }
