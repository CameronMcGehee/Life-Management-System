<?php
	...
	class calendarEventCalendar extends render {
		...
		function render() {
			...
			$calendarEvents = getActiveCalendarEventInstances($month_ini->format('Y-m-d'), $month_end->format('Y-m-d'));
			$completedCalendarEvents = getCompletedCalendarEvents($month_ini->format('Y-m-d'), $month_end->format('Y-m-d'));
            ...
            if ($this->options['showAdd'] || $this->options['showMonthSelector']) {
                ...
                if ($this->options['showAdd']) {
                    $this->output .= '<a class="smallButtonWrapper greenButton noUnderline yCenteredFlex" href="'.$this->options['rootPathPrefix'].'admin/calendarEvents/calendarEvent">➕ Schedule CalendarEvent</a>';
                }
                $this->output .= '</div>';
			...
			while (!$lastDayHit) {
				...
				if ($firstDayHit) {
					...
					foreach ($completedCalendarEvents as $currentCalendarEvent) {
						$currentCalendarEvent['instanceDate'] = strval($currentCalendarEvent['instanceDate']);
						...
						if ($currentCalendarEventStartDateTime->format('Y-m-d') == $currentDate->format('Y-m-d')) { // If it is today
							$calendarEventsOutput .= '<a href="'.$this->options['rootPathPrefix'].'admin/calendarEvents/completedcalendarEvent?id='.htmlspecialchars($currentCalendarEvent['completedCalendarEventId']).'&instance='.$currentCalendarEvent['instanceDate'].'"><p class="calendarEvent completedCalendarEvent">'.htmlspecialchars($currentCalendarEvent['name']).'</p></a>';
						}
                    }

                    foreach ($calendarEvents as $currentCalendarEvent) {
						$currentCalendarEvent['instanceDate'] = strval($currentCalendarEvent['instanceDate']);
						$currentCalendarEvent['endDateTime'] = strval($currentCalendarEvent['endDateTime']);
						$currentCalendarEventStartDateTime = new DateTime($currentCalendarEvent['instanceDate']);
							...
							if (!$currentCalendarEvent['isCompleted']) {
								if ($currentCalendarEvent['isCancelled']) {
									if ($currentCalendarEvent['frequencyInterval'] == 'none') {
										$calendarEventsOutput .= '<a href="'.$this->options['rootPathPrefix'].'admin/calendarEvents/calendarEvent?id='.htmlspecialchars($currentCalendarEvent['calendarEventId']).'&instance='.$currentCalendarEvent['instanceDate'].'"><p class="calendarEvent cancelledCalendarEvent">'.htmlspecialchars($currentCalendarEvent['name']).'</p></a>';
									} else {
										$calendarEventsOutput .= '<a href="'.$this->options['rootPathPrefix'].'admin/calendarEvents/instance?id='.htmlspecialchars($currentCalendarEvent['calendarEventId']).'&instance='.$currentCalendarEvent['instanceDate'].'"><p class="calendarEvent cancelledCalendarEvent">'.htmlspecialchars($currentCalendarEvent['name']).'</p></a>';
									}
								} elseif ($currentCalendarEvent['isRescheduled']) {
									$calendarEventsOutput .= '<a href="'.$this->options['rootPathPrefix'].'admin/calendarEvents/instance?id='.htmlspecialchars($currentCalendarEvent['calendarEventId']).'&instance='.$currentCalendarEvent['instanceDate'].'"><p class="calendarEvent activeCalendarEvent">'.htmlspecialchars($currentCalendarEvent['name']).'</p></a>';
								} else {
									$calendarEventsOutput .= '<a href="'.$this->options['rootPathPrefix'].'admin/calendarEvents/calendarEvent?id='.htmlspecialchars($currentCalendarEvent['calendarEventId']).'&instance='.$currentCalendarEvent['instanceDate'].'"><p class="calendarEvent activeCalendarEvent">'.htmlspecialchars($currentCalendarEvent['name']).'</p></a>';
