<?php
	...
	class customerJobCalendar extends render {
		...
		function __construct(string $renderId, array $options = []) {
			...
			if (!isset($options['businessId'])) {
				if (isset($_SESSION['ultiscape_businessId'])) {
                    ...
                    throw new Exception("No businessId set to pull invoices from (in customerInvoiceTable)");
                }
			}

            if (!isset($options['customerId'])) {
				if (isset($_SESSION['ultiscape_customerId'])) {
                    $options['customerId'] = $_SESSION['ultiscape_customerId'];
                } else {
                    throw new Exception("No customerId set to pull invoices from (in customerInvoiceTable)");
                }
			}
		...
		function render() {
			...
			$jobs = getActiveJobInstances($month_ini->format('Y-m-d'), $month_end->format('Y-m-d'), "AND linkedToCustomerId = '".$this->options['customerId']."'");
			$completedJobs = getCompletedJobs($month_ini->format('Y-m-d'), $month_end->format('Y-m-d'), "AND linkedToCustomerId = '".$this->options['customerId']."'");
				...
				if ($firstDayHit) {
					...
					foreach ($completedJobs as $currentJob) {
						...
						if ($currentJobStartDateTime->format('Y-m-d') == $currentDate->format('Y-m-d')) { // If it is today
							$jobsOutput .= '<a href="'.$this->options['rootPathPrefix'].'customer/jobs/completedjob?id='.htmlspecialchars($currentJob['completedJobId']).'&instance='.$currentJob['instanceDate'].'"><p class="job completedJob">'.htmlspecialchars($currentJob['name']).'</p></a>';
						}
                    }
							...
							if (!$currentJob['isCompleted']) {
								if ($currentJob['isCancelled']) {
									if ($currentJob['frequencyInterval'] == 'none') {
										$jobsOutput .= '<a href="'.$this->options['rootPathPrefix'].'customer/jobs/job?id='.htmlspecialchars($currentJob['jobId']).'&instance='.$currentJob['instanceDate'].'"><p class="job cancelledJob">'.htmlspecialchars($currentJob['name']).'</p></a>';
									} else {
										$jobsOutput .= '<a href="'.$this->options['rootPathPrefix'].'customer/jobs/instance?id='.htmlspecialchars($currentJob['jobId']).'&instance='.$currentJob['instanceDate'].'"><p class="job cancelledJob">'.htmlspecialchars($currentJob['name']).'</p></a>';
									}
								} elseif ($currentJob['isRescheduled']) {
									$jobsOutput .= '<a href="'.$this->options['rootPathPrefix'].'customer/jobs/instance?id='.htmlspecialchars($currentJob['jobId']).'&instance='.$currentJob['instanceDate'].'"><p class="job activeJob">'.htmlspecialchars($currentJob['name']).'</p></a>';
								} else {
									$jobsOutput .= '<a href="'.$this->options['rootPathPrefix'].'customer/jobs/job?id='.htmlspecialchars($currentJob['jobId']).'&instance='.$currentJob['instanceDate'].'"><p class="job activeJob">'.htmlspecialchars($currentJob['name']).'</p></a>';
