<?php
    ...
    class customerEstimateTable extends render {
        ...
        function __construct(string $renderId, array $options = []) {
            ...
            if (!isset($options['businessId'])) {
				if (isset($_SESSION['ultiscape_businessId'])) {
                    ...
                    throw new Exception("No businessId set to pull estimates from (in customerEstimateTable)");
                }
			}
            ...
            if (!isset($options['customerId'])) {
				if (isset($_SESSION['ultiscape_customerId'])) {
                    ...
                    throw new Exception("No customerId set to pull estimates from (in customerEstimateTable)");
                }
			}
        ...
        function render() {
            ...
            $pageCountQuery = "WHERE businessId = '".$_SESSION['ultiscape_businessId']."' AND customerId = '".$_SESSION['ultiscape_customerId']."'";
            ...
            if ($this->options['showSort'] || $this->options['showPageNav']) {
            ...
            // Get actual results
            $params = " AND customerId = '".$_SESSION['ultiscape_customerId']."'";
			...
			foreach ($this->currentBusiness->estimates as $estimateId) {
                ...
                $this->output .= '<td class="la nrb vam" style="max-width: 10em;"><a href="'.$this->options['rootPathPrefix'].'customer/estimates/estimate?id='.htmlspecialchars(htmlspecialchars($estimate->estimateId)).'" style="font-size: 1.1em; margin-right: .5em;"><b>'.$docIdOutput.'</b></a></td>
                ';
                ...
                <script>
                    ...
                    function '.$this->renderId.'batchOperation() {
                        ...
                        if ($("#batchSelect option:selected").val() == "delete" && checkedArray.length > 0) {
                                $("#scriptLoader").load("'.$this->options['rootPathPrefix'].'customer/scripts/async/estimate/deleteEstimates.php", {"estimates[]": checkedArray, "authToken": deleteEstimatesAuthToken}, function() {
