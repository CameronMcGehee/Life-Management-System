<?php
    ...
    class customerInvoiceTable extends render {
        ...
        function render() {
			...
			foreach ($this->currentBusiness->invoices as $invoiceId) {
                ...
                $this->output .= '<td class="la nrb vam" style="max-width: 10em;"><a href="'.$this->options['rootPathPrefix'].'customer/invoices/invoice?id='.htmlspecialchars(htmlspecialchars($invoice->invoiceId)).'" style="font-size: 1.1em; margin-right: .5em;"><b>'.$docIdOutput.'</b></a></td>
                ';
                ...
                <script>
                    ...
                    function '.$this->renderId.'batchOperation() {
                        ...
                        if ($("#batchSelect option:selected").val() == "delete" && checkedArray.length > 0) {
                                $("#scriptLoader").load("'.$this->options['rootPathPrefix'].'customer/scripts/async/invoice/deleteInvoices.php", {"invoices[]": checkedArray, "authToken": deleteInvoicesAuthToken}, function() {
