<?php
    ...
    class contactInvoiceTable extends render {
        ...
        function render() {
            ...
            $pageCountQuery = "WHERE workspaceId = '".$_SESSION['lifems_workspaceId']."' AND contactId = '".$_SESSION['lifems_contactId']."'";
            ...
            // Get actual results
            $params = " AND contactId = '".$_SESSION['lifems_contactId']."'";
			...
			foreach ($this->currentWorkspace->invoices as $invoiceId) {
                ...
                $paymentStatusOutput = '<div style="display: inline-block; width: .9em; height: .9em; border-radius: 50%; background-color: '.$paymentStatusColor.'"></div> <b>'.htmlspecialchars(strval($this->currentWorkspace->currencySymbol)).$amountPaid.'</b> ('.$percentPaid.'%)';
                ...
                $this->output .= '<td class="la nrb vam" style="max-width: 10em;"><a href="'.$this->options['rootPathPrefix'].'contact/invoices/invoice?id='.htmlspecialchars(htmlspecialchars($invoice->invoiceId)).'" style="font-size: 1.1em; margin-right: .5em;"><b>'.$docIdOutput.'</b></a></td>
                ';
                ...
                <script>
                    ...
                    function '.$this->renderId.'batchOperation() {
                        ...
                        if ($("#batchSelect option:selected").val() == "delete" && checkedArray.length > 0) {
                                $("#scriptLoader").load("'.$this->options['rootPathPrefix'].'contact/scripts/async/invoice/deleteInvoices.php", {"invoices[]": checkedArray, "authToken": deleteInvoicesAuthToken}, function() {
