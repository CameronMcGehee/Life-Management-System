...
// Javascript parser would fail on any of such named variable coming from PHP
const default_map = {
    'var': 'var_r',
    break: 'break_r',
    case: 'case_r',
...
function mapIdent(name) {
    if (name in map && name !== 'constructor') {
        // console.log(`Transforming from ${name} to ${map[name]}`);
        return map[name];
    }
    return name;
}
...
function translateVariable(node, scope) {
    ...
    if ('string' === typeof node.name) {
        ...
        return makeIdent(mapIdent(newName), node.loc);
    } else {
        return {
...
function translateNamespace(node, scope) {
    assert(scope instanceof Scope);
    var body = node.children.map(e => translateBlockLike(e, scope));

    var tbody = body.filter(Boolean);
    if (!encapsulate_ns && tbody.length == 1 && tbody[0] && tbody[0].expression) {
        return tbody[0].expression;
    }
...
function translateBinary(node, scope) {
    ...
    const operator = node.type === '.' ? '+' : (node.type == '??' ? '||' : node.type);
    return {
        type: "BinaryExpression",
        operator,
        left: translateExpr(node.left, scope),
...
function translateCast(node, scope) {
    switch(node.type) {
        ...
        case 'array': return {
            ...
            arguments: [joinWithOperator([translateExpr(node.what, scope), makeString("", node.loc)], "||")]
        };
        default: return {
            type: "CallExpression",
            loc: translateLoc(node.loc),
...
function translateStaticLookup(node, scope) {
    ...
    if ('\\self' === name) {
        ...
        return {
            type: "MemberExpression",
            loc: translateLoc(node.loc),
            object: makeIdent(scope.className, node.loc),
...
function translateParameter(node, scope) {
    ...
    var name = mapIdent(node.name);
    scope.declareVar(name);
    const ident = makeIdent(name, node.loc);
    ...
    return {
        ...
        declarations: names.map(name => {
            ...
            if ('string' === typeof name) {
                ...
                id = makeIdent(mapIdent(newName), loc);
            } else {
                id = name;
            }
            return {
