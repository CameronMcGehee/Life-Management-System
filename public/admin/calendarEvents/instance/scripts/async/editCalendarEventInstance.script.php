<?php
	...
	if (!$currentInstance->existed || $currentInstance->workspaceId != $_SESSION['lifems_workspaceId']) {
	...
	if (!isset($formData['mainAuthToken']) || !validateAuthToken($formData['mainAuthToken'], 'editCalendarEventInstanceException')) {
		echo 'tokenInvalid';
		exit();
	}
	...
	function setValuesFromForm($formData) {
		...
		if (!isset($formData['contact']) || empty($formData['contact'])) {
			...
			if ($currentContact->existed) {
				$currentInstance->linkedToContactId = $formData['contact'];
			} else {
				$currentInstance->linkedToContactId = 'NULL';
			}
		}
	...
	useAuthToken($formData['mainAuthToken'], 'editCalendarEventInstanceException');
