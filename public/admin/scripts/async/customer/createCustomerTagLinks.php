<?php
	...
	if (!validateAuthToken($_POST['authToken'], 'createCustomerTagLinks')) {
		echo 'tokenInvalid';
		exit();
	}
    ...
    foreach ($_POST['customerTagLinks'] as $tagLink) {
        ...
        // create the link
        if ($db->insert('customerCustomerTagBridge', ['businessId' => $_SESSION['ultiscape_businessId'], 'customerId' => $db->sanitize($tagLink[0]), 'customerTagId' => $db->sanitize($tagLink[1]))) {
            echo 'success';
        } else {
            echo 'insertError';
            die();
	...
	useAuthToken($_POST['authToken'], 'createCustomerTagLinks');
