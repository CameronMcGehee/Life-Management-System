<?php
    ...
    foreach ($phonesReceived as $currentReceivedPhoneNumber) {
        $dbPhone = new customerPhoneNumber($currentReceivedPhoneNumber['id']);
        if ($dbPhone->existed && $dbPhone->customerId == $currentCustomer->customerId) {
    ...
    if (!empty($formData['newPhoneNumber'])) {
        ...
        $newEmail = new customerPhoneNumber();
